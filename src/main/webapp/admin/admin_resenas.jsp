<!DOCTYPE html>
<html lang="es">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Gestión de Reseñas - Admin</title>
    <style>
        * {
            margin: 0;
            padding: 0;
            box-sizing: border-box;
            font-family: Arial, sans-serif;
        }

        body {
            background-color: #1a1a1a;
            display: flex;
            justify-content: center;
            align-items: center;
            min-height: 100vh;
        }

        .contenedor {
            display: flex;
            background-color: #ffffff;
            width: 100%;
            max-width: 1200px;
            height: 90vh;
            border: 1px solid #ccc;
        }

        .barra-lateral {
            width: 250px;
            border-right: 1px solid #eaeaea;
            padding: 20px;
            display: flex;
            flex-direction: column;
        }

        .barra-lateral h3 {
            font-size: 14px;
            letter-spacing: 1px;
            margin-bottom: 10px;
        }

        .separador {
            border: none;
            border-top: 1px solid #eaeaea;
            margin: 15px 0;
        }

        .barra-lateral ul {
            list-style: none;
        }

        .barra-lateral li {
            margin-bottom: 20px;
        }

        .barra-lateral a {
            text-decoration: none;
            color: #000;
            font-size: 14px;
        }

        .contenido-principal {
            flex: 1;
            display: flex;
            flex-direction: column;
        }

        .cabecera {
            display: flex;
            align-items: center;
            padding: 15px 20px;
            border-bottom: 1px solid #eaeaea;
            gap: 10px;
        }

        .titulo-cabecera {
            margin-right: auto;
            font-size: 16px;
            font-weight: bold;
        }

        .btn {
            padding: 5px 15px;
            border: 1px solid #000;
            background: #fff;
            cursor: pointer;
            font-size: 12px;
        }

        .seccion-contenido {
            padding: 40px;
            flex: 1;
            overflow-y: auto;
        }

        .seccion-contenido h1 {
            font-size: 28px;
            font-weight: normal;
            margin-bottom: 5px;
            text-transform: uppercase;
        }

        .linea-titulo {
            border: none;
            border-top: 2px solid #ccc;
            width: 50px;
            margin-bottom: 20px;
        }

        .subtitulo-descripcion {
            font-size: 10px;
            color: #777;
            text-transform: uppercase;
            letter-spacing: 1px;
            margin-bottom: 30px;
        }

        .tabla-admin {
            width: 100%;
            border-collapse: collapse;
            margin-bottom: 30px;
        }

        .tabla-admin th, .tabla-admin td {
            border-bottom: 1px solid #eaeaea;
            padding: 20px 15px;
            text-align: left;
            font-size: 13px;
        }

        .tabla-admin th {
            font-size: 10px;
            font-weight: bold;
            text-transform: uppercase;
        }

        .celda-comentario {
            color: #555;
            font-style: italic;
            max-width: 300px;
        }

        .celda-calificacion {
            font-weight: bold;
        }

        .estrella {
            font-size: 10px;
            color: #000;
            margin-left: 2px;
        }

        .btn-tabla {
            padding: 6px 12px;
            border: 1px solid #000;
            background-color: #fff;
            color: #000;
            font-size: 9px;
            font-weight: bold;
            cursor: pointer;
            text-transform: uppercase;
        }

        .btn-tabla:hover {
            background-color: #f0f0f0;
        }

        .contenedor-paginacion {
            display: flex;
            justify-content: space-between;
            align-items: center;
            border-top: 1px solid #eaeaea;
            padding-top: 20px;
        }

        .texto-paginacion {
            font-size: 10px;
            color: #777;
            text-transform: uppercase;
            letter-spacing: 1px;
        }

        .controles-paginacion {
            display: flex;
            gap: 5px;
        }

        .btn-pagina {
            padding: 5px 10px;
            border: 1px solid #eaeaea;
            background-color: #fff;
            color: #000;
            font-size: 11px;
            font-weight: bold;
            cursor: pointer;
        }

        .btn-pagina.activo {
            background-color: #000;
            color: #fff;
            border-color: #000;
        }

        .pie-pagina {
            padding: 15px;
            text-align: center;
            border-top: 1px solid #eaeaea;
            font-size: 10px;
            letter-spacing: 1px;
            color: #777;
        }
    </style>
</head>
<body>

    <div class="contenedor">
        
        <aside class="barra-lateral">
            <h3>ADMINISTRACIÓN</h3>
            <hr class="separador">
            <ul>
                <li><a href="admin.html">Panel Principal</a></li>
                <li><a href="admin_usuarios.html">Usuarios</a></li>
                <li><a href="admin_productos.html">Productos</a></li>
                <li><a href="admin_pedidos.html">Pedidos</a></li>
                <li><a href="admin_resenas.html" style="font-weight: bold;">Reseñas</a></li>
            </ul>
            <hr class="separador">
            <ul>
                <li><a href="index.html">Ver Tienda</a></li>
                <li><a href="login.html">Cerrar Sesión</a></li>
            </ul>
        </aside>

        <main class="contenido-principal">
            
            <header class="cabecera">
                <div class="titulo-cabecera">PANEL DE ADMINISTRACIÓN</div>
                <button class="btn">Perfil Admin</button>
                <button class="btn" onclick="window.location.href='login.html'">Cerrar Sesión</button>
            </header>

            <section class="seccion-contenido">
                
                <h1>GESTIÓN DE RESEÑAS</h1>
                <hr class="linea-titulo">
                <p class="subtitulo-descripcion">Administre y modifique las opiniones </p>

                <table class="tabla-admin">
                    <thead>
                        <tr>
                            <th>USUARIO</th>
                            <th>PRODUCTO</th>
                            <th>CALIFICACIÓN</th>
                            <th>COMENTARIO</th>
                            <th>FECHA</th>
                            <th>ACCIONES</th>
                        </tr>
                    </thead>
                    <tbody>
                        <tr>
                            <td>Juan Pérez</td>
                            <td>Smartphone X</td>
                            <td class="celda-calificacion">5.0 </td>
                            <td class="celda-comentario">Excelente producto</td>
                            <td>2023-10-01</td>
                            <td>
                                <button class="btn-tabla">ELIMINAR</button>
                            </td>
                        </tr>
                        <tr>
                            <td>María García</td>
                            <td>Laptop Pro</td>
                            <td class="celda-calificacion">4.0 </td>
                            <td class="celda-comentario">Buena relación calidad-precio</td>
                            <td>2023-10-02</td>
                            <td>
                                <button class="btn-tabla">ELIMINAR</button>
                            </td>
                        </tr>
                        <tr>
                            <td>Carlos Ruiz</td>
                            <td>Auriculares BT</td>
                            <td class="celda-calificacion">2.0 </td>
                            <td class="celda-comentario">Se desconectan frecuentemente de no se</td>
                            <td>2023-10-03</td>
                            <td>
                                <button class="btn-tabla">ELIMINAR</button>
                            </td>
                        </tr>
                        <tr>
                            <td>Ana López</td>
                            <td>Reloj Inteligente</td>
                            <td class="celda-calificacion">5.0</td>
                            <td class="celda-comentario">Me encanta el diseño y la duración a</td>
                            <td>2023-10-04</td>
                            <td>
                                <button class="btn-tabla">ELIMINAR</button>
                            </td>
                        </tr>
                        <tr>
                            <td>Roberto Díaz</td>
                            <td>Cámara 4K</td>
                            <td class="celda-calificacion">3.0 </td>
                            <td class="celda-comentario">La calidad de imagen es buena per</td>
                            <td>2023-10-05</td>
                            <td>
                                <button class="btn-tabla">ELIMINAR</button>
                            </td>
                        </tr>
                    </tbody>
                </table>

                <div class="contenedor-paginacion">
                    <div class="texto-paginacion">MOSTRANDO 5 DE 128 RESEÑAS</div>
                    <div class="controles-paginacion">
                        <button class="btn-pagina">&lt;</button>
                        <button class="btn-pagina activo">1</button>
                        <button class="btn-pagina">2</button>
                        <button class="btn-pagina">3</button>
                        <button class="btn-pagina">&gt;</button>
                    </div>
                </div>

            </section>

            <footer class="pie-pagina">
                <p>APLICACIONES WEB – UNIDAD 2</p>
            </footer>

        </main>

    </div>

</body>
</html>